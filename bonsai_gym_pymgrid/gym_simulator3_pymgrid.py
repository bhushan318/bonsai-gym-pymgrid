...
from pymgrid import MicrogridGenerator as Mgen  # create random sim configuration
import gym
...
class GymSimulator3(SimulatorSession):
    ...
    def __init__(
        self, config: BonsaiClientConfig, iteration_limit: int = 0, skip_frame: int = 1,) -> None:
        super(GymSimulator3, self).__init__(config)
        config_ = Mgen.MicrogridGenerator(nb_microgrid=1)  # number of microgrids
        config_.generate_microgrid(verbose=False)
        # create and reset the gym environment
        self._env = MicroGridEnv({'microgrid': config_.microgrids[0]})
        self._env.seed(20)
